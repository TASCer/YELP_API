...
def get_snippets(response):
    ...
    for business in response['businesses']:
        ...
        List_review_URL.append(URL)
    for url in List_review_URL:
        #print(url)
        review_header = {'Authorization': 'Bearer %s' % bearer_token}
        review_response = requests.request('GET', url, headers=review_header, params=params)
        all_snippets.append({id, name, review_response.text})
        #print(all_snippets)
